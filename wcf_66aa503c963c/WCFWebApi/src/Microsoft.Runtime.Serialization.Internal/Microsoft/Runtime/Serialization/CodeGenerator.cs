...
namespace Microsoft.Runtime.Serialization
{
    ...
    class CodeGenerator
    {
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", Justification = "This is cloned code.")]
        ...
        internal void VerifyParameterCount(MethodInfo methodInfo, int expectedCount)
        {
            if (methodInfo.GetParameters().Length != expectedCount)
                throw Fx.Exception.AsError(new SerializationException(SR.ParameterCountMismatch(methodInfo.Name, methodInfo.GetParameters().Length, expectedCount)));
        }
        ...
        internal Type LoadMember(MemberInfo memberInfo)
        {
            ...
            else if (memberInfo.MemberType == MemberTypes.Property)
            {
                ...
                if (property != null)
                {
                    ...
                    if (getMethod == null)
                        throw Fx.Exception.AsError(new SerializationException(SR.NoGetMethodForProperty(property.DeclaringType, property)));
                    Call(getMethod);
                }
            }
            else if (memberInfo.MemberType == MemberTypes.Method)
            {
                ...
                throw Fx.Exception.AsError(new SerializationException(SR.CannotLoadMemberType(memberInfo.MemberType)));
        ...
        internal void StoreMember(MemberInfo memberInfo)
        {
            ...
            else if (memberInfo.MemberType == MemberTypes.Property)
            {
                ...
                if (property != null)
                {
                    ...
                    if (setMethod == null)
                        throw Fx.Exception.AsError(new SerializationException(SR.NoSetMethodForProperty(property.DeclaringType, property)));
                    Call(setMethod);
                }
            }
            else if (memberInfo.MemberType == MemberTypes.Method)
        ...
        internal void Store(object var)
        {
            ...
            else
            {
                ...
                throw Fx.Exception.AsError(new SerializationException(SR.CanOnlyStoreIntoArgOrLocGot0(DataContract.GetClrTypeFullName(var.GetType()))));
            }
        }
            ...
            else
            {
                switch (Type.GetTypeCode(valueType))
                {
                    ...
                    case TypeCode.Char:
                        ...
                        throw Fx.Exception.AsError(new NotSupportedException(SR.CharIsInvalidPrimitive));
                    case TypeCode.SByte:
                    case TypeCode.Byte:
                    case TypeCode.Int16:
                    case TypeCode.UInt16:
                    ...
                    default:
                        throw Fx.Exception.AsError(new SerializationException(SR.UnknownConstantType(DataContract.GetClrTypeFullName(valueType))));
                }
            }
        }
        ...
        internal void Ldelem(Type arrayElementType)
        {
            ...
            else
            {
                ...
                if (opCode.Equals(OpCodes.Nop))
                    throw Fx.Exception.AsError(new SerializationException(SR.ArrayTypeIsNotSupported(DataContract.GetClrTypeFullName(arrayElementType))));
                if (codeGenTrace != CodeGenTrace.None)
                    EmitSourceInstruction(opCode.ToString());
                ilGen.Emit(opCode);
        ...
        void InternalConvert(Type source, Type target, bool isAddress)
        {
            ...
            if (target.IsValueType)
            {
                if (source.IsValueType)
                {
                    ...
                    if (opCode.Equals(OpCodes.Nop))
                        throw Fx.Exception.AsError(new SerializationException(SR.NoConversionPossibleTo(DataContract.GetClrTypeFullName(target))));
                    else
                    {
                        if (codeGenTrace != CodeGenTrace.None)
                            EmitSourceInstruction(opCode.ToString());
                ...
                else if (source.IsAssignableFrom(target))
                {
                    ...
                    throw Fx.Exception.AsError(new SerializationException(SR.IsNotAssignableFrom(DataContract.GetClrTypeFullName(target), DataContract.GetClrTypeFullName(source))));
            }
        ...
        {
            throw Fx.Exception.AsError(new SerializationException(SR.ExpectingEnd(expected.ToString())));
        }
        ...
        internal void StoreOutParam(ArgBuilder arg, object value)
        {
            ...
            if (!destType.IsByRef)
                throw Fx.Exception.AsError(new SerializationException(SR.GetString(SR.OutParametersMustBeByRefTypeReceived, DataContract.GetClrTypeFullName(destType))));
