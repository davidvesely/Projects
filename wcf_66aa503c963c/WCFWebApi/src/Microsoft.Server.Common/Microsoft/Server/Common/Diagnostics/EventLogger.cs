...
namespace Microsoft.Server.Common.Diagnostics
{
    ...
    internal sealed class EventLogger
    {
        ...
        [SecurityCritical]
        internal static EventLogger UnsafeCreateEventLogger(string eventLogSourceName, DiagnosticTraceBase diagnosticTrace)
        {
        ...
        internal void UnsafeLogEvent(TraceEventType type, ushort eventLogCategory, uint eventId, bool shouldTrace, params string[] values)
        {
        ...
        internal void LogEvent(TraceEventType type, ushort eventLogCategory, uint eventId, bool shouldTrace, params string[] values)
        {
            if (canLogEvent)
            {
                try
                ...
                catch (SecurityException ex)
                {
                    ...
                    {
                        this.diagnosticTrace.TraceHandledException(ex);
                    }
                }
            }
        }

        internal void LogEvent(TraceEventType type, ushort eventLogCategory, uint eventId, params string[] values)
        {
        ...
        [SecuritySafeCritical]
        [SecurityPermission(SecurityAction.Demand, UnmanagedCode = true)]
        void SafeSetLogSourceName(string name)
        {
            this.eventLogSourceName = name;
        }

        [Fx.Tag.SecurityNote(Critical = "Sets event source name.")]
        [SecurityCritical]
        void SetLogSourceName(string name, DiagnosticTraceBase diagnosticTraceBase)
        {
            this.eventLogSourceName = name;
            this.diagnosticTrace = diagnosticTraceBase;
        }
