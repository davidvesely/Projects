...
#else
            string expected = string.Format(CultureInfo.InvariantCulture, "[{0},null,{1}]", item1.ToString(JsonSaveOptions.None), item3.ToString(JsonSaveOptions.None));
            Assert.AreEqual(expected, target.ToString(JsonSaveOptions.None));
#endif
...
#else
            Assert.AreEqual(json, target.ToString(JsonSaveOptions.EnableIndent));
