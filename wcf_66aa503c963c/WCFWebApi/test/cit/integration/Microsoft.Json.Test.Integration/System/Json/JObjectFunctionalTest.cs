    ...
    public class JObjectFunctionalTest
    {
        ...
        public void JsonObjectItemsFunctionalTest()
        {
            ...
            for (int i = 0; i < iterationCount / 10; i++)
            {
                ...
                if (sourceJson["1"].ToString(JsonSaveOptions.None) != "true")
                {
                ...
                do
                {
                    actual.Add(String.Format("{0} - {1}", ko.Current.Key, ko.Current.Value.ToString(JsonSaveOptions.None)));
                ...
                {
                    expectedList.Add(v.ToString(JsonSaveOptions.None));
                }
                ...
                {
                    actualList.Add(v.ToString(JsonSaveOptions.None));
                }
            ...
            for (int i = 0; i < numberOfThreads; i++)
            {
                threads[i] = new Thread(new ThreadStart(delegate
                {
                    for (int j = 0; j < 10; j++)
                    {
                        try
                        {
                            string str = jo.ToString(JsonSaveOptions.None);
                            if (str != expected)
                            {
                                incorrectValue = true;
