...
namespace System.Net.Http
{
    ...
    internal class HttpResponseHeaderParser
    {
        ...
        private HttpResponseState responseStatus = HttpResponseState.StatusLine;

        private HttpStatusLineParser statusLineParser;
        ...
        public HttpResponseHeaderParser(HttpUnsortedResponse httpResponse, int maxResponseLineSize, int maxHeaderSize)
        {
            ...
            this.statusLineParser = new HttpStatusLineParser(this.httpResponse, maxResponseLineSize);

            // Create header parser
            this.headerParser = new InternetMessageFormatHeaderParser(this.httpResponse.HttpHeaders, maxHeaderSize);
        }
            ...
            ref int bytesConsumed)
        {
            ...
            switch (this.responseStatus)
            {
                case HttpResponseState.StatusLine:
                    try
                    {
                        subParseStatus = this.statusLineParser.ParseBuffer(buffer, bytesReady, ref bytesConsumed);
                    }
                    catch (Exception)
                    {
                    ...
                    {
                        this.responseStatus = HttpResponseState.ResponseHeaders;
