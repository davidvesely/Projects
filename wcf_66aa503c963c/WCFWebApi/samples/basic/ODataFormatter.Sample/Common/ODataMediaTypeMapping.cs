...
namespace ODataFormatter.Sample
{
    ...
    internal class ODataMediaTypeMapping : MediaTypeMapping
    {
        ...
        protected override sealed double OnTryMatchMediaType(HttpResponseMessage response)
        {
            ...
            return String.Equals(responseMediaType.ToString(), MediaType.ToString(), StringComparison.OrdinalIgnoreCase) && this.IsQueryStringMatch(queryString)
                        ? quality
                        : 0.0;
        }
        ...
        private bool IsQueryStringMatch(NameValueCollection queryString)
        {
            if (queryString != null)
            {
