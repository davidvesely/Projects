...
namespace System.Json
{
    ...
    public class JsonValue : IEnumerable<KeyValuePair<string, JsonValue>>, IDynamicMetaObjectProvider
    {
        ...
        public virtual JsonValue this[string key]
        {
            get
            {
                throw Fx.Exception.AsError(new InvalidOperationException(SR.IndexerNotSupportedOnJsonType(typeof(string), this.JsonType)));
            }
        ...
        public virtual JsonValue this[int index]
        {
            get
            {
                throw Fx.Exception.AsError(new InvalidOperationException(SR.IndexerNotSupportedOnJsonType(typeof(int), this.JsonType)));
            }
        ...
        public static JsonValue Load(TextReader textReader)
        {
            if (textReader == null)
            {
                throw Fx.Exception.ArgumentNull("textReader");
            }
        ...
        public static T CastValue<T>(JsonValue value)
        {
            ...
            if (value == null || value.JsonType == JsonType.Default)
            {
                ...
                {
                    throw Fx.Exception.AsError(new InvalidCastException(SR.InvalidCastNonNullable(typeofT.FullName)));
                }
                else
                {
                    return default(T);
            ...
            catch (Exception ex)
            {
                ...
                {
                    throw Fx.Exception.AsError(new InvalidCastException(SR.CannotCastJsonValue(value.GetType().FullName, typeofT.FullName), ex));
                }
        ...
        public virtual object ReadAs(Type type)
        {
            ...
            throw Fx.Exception.AsError(new NotSupportedException(SR.CannotReadAsType(this.GetType().FullName, type.FullName)));
        }
        ...
        {
            this.Save(stream, JsonSaveOptions.None);
        }

        /// <summary>
        /// Writes this <see cref="System.Json.JsonValue"/> instance to a <see cref="System.IO.Stream"/> using the provided <see cref="JsonSaveOptions"/>.
        /// </summary>
        /// <param name="stream">Stream to which to write text-based JSON.</param>
        /// <param name="options">The <see cref="JsonSaveOptions"/> serialization options.</param>
        public void Save(Stream stream, JsonSaveOptions options)
        {
            if (this.JsonType == JsonType.Default)
            {
                throw Fx.Exception.AsError(new InvalidOperationException(SR.UseOfDefaultNotAllowed));
            }

            JsonSaveOptionsHelper.Validate(options, "options");
            ...
            bool enableIndent = (options & JsonSaveOptions.EnableIndent) == JsonSaveOptions.EnableIndent;
            if (enableIndent)
            {
                // TODO, CSDMain 228569, remove the following once we moved to the simple text writer, instead of using XmlJsonWriter
        ...
        {
            this.Save(textWriter, JsonSaveOptions.None);
        }

        /// <summary>
        /// Writes this <see cref="System.Json.JsonValue"/> instance to a <see cref="TextWriter"/> using the provided <see cref="JsonSaveOptions"/>.
        /// </summary>
        /// <param name="textWriter">The <see cref="System.IO.TextWriter"/> used to write text-based JSON.</param>
        /// <param name="options">The <see cref="JsonSaveOptions"/> serialization options.</param>
        public void Save(TextWriter textWriter, JsonSaveOptions options)
        {
            ...
            if (textWriter == null)
            {
                throw Fx.Exception.ArgumentNull("textWriter");
            }

            using (MemoryStream ms = new MemoryStream())
            {
                this.Save(ms, options);
                ms.Position = 0;
        ...
        {
            return this.ToString(JsonSaveOptions.None);
        }

        /// <summary>
        /// Provides a textual representation of this <see cref="System.Json.JsonValue"/> instance using the provided <see cref="JsonSaveOptions"/>.
        /// </summary>
        /// <param name="options">A combination of <see cref="JsonSaveOptions"/> values that specifies the formatting behavior.</param>
        /// <returns>A <see cref="System.String"/> containing text-based JSON formatted according to the <paramref name="options"/>.</returns>
        public string ToString(JsonSaveOptions options)
        {
            ...
            if (enableIndent)
            {
                return JsonValueExtensions.PrettyPrint(this);
            }
            else
            {
                using (MemoryStream ms = new MemoryStream())
        ...
        public JsonValue ValueOrDefault(params object[] indexes)
        {
            ...
            for (int i = 0; i < indexes.Length; i++)
            {
                ...
                switch (Type.GetTypeCode(indexType))
                {
                    ...
                    default:
                        throw Fx.Exception.Argument("indexes", SR.InvalidIndexType(index.GetType()));
                }
            }
        ...
        DynamicMetaObject IDynamicMetaObjectProvider.GetMetaObject(Expression parameter)
        {
            if (parameter == null)
            {
                throw Fx.Exception.ArgumentNull("parameter");
            }
        ...
        internal static JsonValue ResolveObject(object value)
        {
            ...
            throw Fx.Exception.Argument("value", SR.TypeNotSupported);
        }
