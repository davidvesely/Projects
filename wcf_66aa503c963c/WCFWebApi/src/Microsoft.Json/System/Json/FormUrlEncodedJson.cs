...
namespace System.Json
{
    ...
    public static class FormUrlEncodedJson
    {
        ...
        private static readonly string[] emptyPath = new string[]
        {
            string.Empty
        };
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is a low-level API used by other APIs to provide end-user functionality.")]
        public static JsonObject Parse(IEnumerable<Tuple<string, string>> nameValuePairs)
        {
        ...
        /// <returns>The <see cref="System.Json.JsonObject"/> corresponding to the given query string values.</returns>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is a low-level API used by other APIs to provide end-user functionality.")]
        public static JsonObject Parse(IEnumerable<Tuple<string, string>> nameValuePairs, int maxDepth)
        {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is a low-level API used by other APIs to provide end-user functionality.")]
        public static bool TryParse(IEnumerable<Tuple<string, string>> nameValuePairs, out JsonObject value)
        {
        ...
        /// <returns><c>true</c> if <paramref name="nameValuePairs"/> was parsed successfully; otherwise false.</returns>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is a low-level API used by other APIs to provide end-user functionality.")]
        public static bool TryParse(IEnumerable<Tuple<string, string>> nameValuePairs, int maxDepth, out JsonObject value)
        {
        ...
        /// <returns>The <see cref="System.Json.JsonObject"/> corresponding to the given query string values.</returns>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is a low-level API used by other APIs to provide end-user functionality.")]
        private static JsonObject ParseInternal(IEnumerable<Tuple<string, string>> nameValuePairs, int maxDepth, bool throwOnError)
        {
            ...
            {
                throw new ArgumentException(SR.MinParameterSize(MinDepth), "maxDepth");
            }
            ...
            foreach (var nameValuePair in nameValuePairs)
            {
                if (nameValuePair.Item1 == null)
                {
                    if (string.IsNullOrEmpty(nameValuePair.Item2))
                    {
                        if (throwOnError)
                        {
                            throw new ArgumentException(SR.QueryStringNameShouldNotNull, "nameValuePairs");
                        }
                ...
                {
                    string[] path = GetPath(nameValuePair.Item1, maxDepth, throwOnError);
                    if (path == null || !Insert(result, path, nameValuePair.Item2, throwOnError))
                    {
                        return null;
                    }
                }
        ...
        private static string[] GetPath(string key, int maxDepth, bool throwOnError)
        {
            ...
            if (path.Length >= maxDepth)
            {
                if (throwOnError)
                {
                    throw new ArgumentException(SR.MaxDepthExceeded(maxDepth));
                }
        ...
        private static bool ValidateQueryString(string key, bool throwOnError)
        {
                ...
                switch (key[i])
                {
                    case '[':
                        ...
                        else
                        {
                            if (throwOnError)
                            {
                                throw new ArgumentException(SR.NestedBracketNotValid(ApplicationFormUrlEncoded, i));
                            }
                    ...
                    case ']':
                        ...
                        else
                        {
                            if (throwOnError)
                            {
                                throw new ArgumentException(SR.UnMatchedBracketNotValid(ApplicationFormUrlEncoded, i));
                            }
            ...
            if (hasUnMatchedLeftBraket)
            {
                if (throwOnError)
                {
                    throw new ArgumentException(SR.NestedBracketNotValid(ApplicationFormUrlEncoded, key.LastIndexOf('[')));
                }
        ...
        private static bool Insert(JsonObject root, string[] path, string value, bool throwOnError)
        {
            ...
            for (int i = 0; i < path.Length - 1; i++)
            {
                if (String.IsNullOrEmpty(path[i]))
                {
                    if (throwOnError)
                    {
                        throw new ArgumentException(SR.InvalidArrayInsert(BuildPathString(path, i)));
                    }
                ...
                else
                {
                    ...
                    if (current[path[i]] == null || current[path[i]] is JsonPrimitive)
                    {
                        if (throwOnError)
                        {
                            throw new ArgumentException(SR.FormUrlEncodedMismatchingTypes(BuildPathString(path, i)));
                        }
            ...
            else
            {
                if (current == null)
                {
                    if (throwOnError)
                    {
                        throw new ArgumentException(SR.FormUrlEncodedMismatchingTypes(BuildPathString(path, path.Length - 1)));
                    }
        ...
        private static bool AddToObject(JsonObject obj, string[] path, string value, bool throwOnError)
        {
            ...
            if (obj.ContainsKey(key))
            {
                if (obj[key] == null)
                {
                    if (throwOnError)
                    {
                        throw new ArgumentException(SR.FormUrlEncodedMismatchingTypes(BuildPathString(path, pathIndex)));
                    }
                ...
                else
                {
                    if (throwOnError)
                    {
                        throw new ArgumentException(SR.JQuery13CompatModeNotSupportNestedJson(BuildPathString(path, pathIndex)));
                    }
        ...
        private static string GetIndex(IEnumerable<string> keys, bool throwOnError)
        {
            ...
            foreach (var key in keys)
            {
                ...
                else
                {
                    if (throwOnError)
                    {
                        throw new ArgumentException(SR.FormUrlEncodedMismatchingTypes(key));
                    }
