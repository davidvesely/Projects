...
namespace System.Net.Http
{
    ...
    /// </summary>
    public static class HttpContentMultipartExtensionMethods
    {
        private const int MinBufferSize = 256;
        private const int DefaultBufferSize = 32 * 1024;

        private static readonly AsyncCallback onMultipartReadAsyncComplete = new AsyncCallback(OnMultipartReadAsyncComplete);
        private static readonly AsyncCallback onMultipartWriteSegmentAsyncComplete = new AsyncCallback(OnMultipartWriteSegmentAsyncComplete);
        ...
        /// <returns>Collection of <see cref="HttpContent"/> instances where each instance represents a body part.</returns>
        public static IEnumerable<HttpContent> ReadAsMultipart(this HttpContent content)
        {
            return ReadAsMultipart(content, MultipartMemoryStreamProvider.Instance, DefaultBufferSize);
        }
        ...
        /// <returns>Collection of <see cref="HttpContent"/> instances where each instance represents a body part.</returns>
        public static IEnumerable<HttpContent> ReadAsMultipart(this HttpContent content, IMultipartStreamProvider streamProvider)
        {
            return ReadAsMultipart(content, streamProvider, DefaultBufferSize);
        }

        /// <summary>
        /// Reads all body parts within a MIME multipart message and produces a set of 
        /// <see cref="HttpContent"/> instances as a result using the <paramref name="streamProvider"/> 
        /// instance to determine where the contents of each body part is written and 
        /// <paramref name="bufferSize"/> as read buffer size.
        /// </summary>
        /// <param name="content">An existing <see cref="HttpContent"/> instance to use for the 
        ...
        /// <returns>Collection of <see cref="HttpContent"/> instances where each instance 
        ...
        public static IEnumerable<HttpContent> ReadAsMultipart(this HttpContent content, IMultipartStreamProvider streamProvider, int bufferSize)
        {
            if (content == null)
            {
                throw new ArgumentNullException("content");
            ...
            {
                throw new ArgumentException(SR.MinParameterSize(MinBufferSize), "bufferSize");
            }
            ...
            List<HttpContent> result = new List<HttpContent>();

            while (true)
            {
                try
                {
                    bytesRead = content.ReadAsStreamAsync().Result.Read(data, 0, data.Length);
                }
                catch (Exception e)
                {
                    parser.Dispose();
                    throw new IOException(SR.ReadAsMimeMultipartErrorReading, e);
                }

                foreach (MimeBodyPart part in parser.ParseBuffer(data, bytesRead))
                {
                    foreach (ArraySegment<byte> segment in part.Segments)
                    {
                        Stream output = part.GetOutputStream();
                        try
                        {
                        ...
                        {
                            parser.Dispose();
                            throw new IOException(SR.ReadAsMimeMultipartErrorWriting, e);
                        }
                    }

                    // If we are done with the part then close it.
                    if (CheckPartCompletion(part, result))
                    {
                        // We are done parsing.
                        return result;
                    }
        ...
        public static Task<IEnumerable<HttpContent>> ReadAsMultipartAsync(this HttpContent content, IMultipartStreamProvider streamProvider, int bufferSize)
        {
            ...
            MimeMultipartBodyPartParser parser = new MimeMultipartBodyPartParser(content, streamProvider);
            byte[] data = new byte[bufferSize];
            MultipartAsyncContext context = new MultipartAsyncContext(content, taskCompletionSource, parser, data);
        ...
        private static void MultipartReadAsync(MultipartAsyncContext context)
        {
            ...
            try
            {
                result = context.Content.ReadAsStreamAsync().Result.BeginRead(context.Data, 0, context.Data.Length, onMultipartReadAsyncComplete, context);
            ...
            {
                Exception exception = (result != null && result.CompletedSynchronously) ? e : new IOException(SR.ReadAsMimeMultipartErrorReading, e);
        ...
        private static void MultipartReadAsyncComplete(IAsyncResult result)
        {
            ...
            try
            {
                bytesRead = context.Content.ReadAsStreamAsync().Result.EndRead(result);
            }
            catch (Exception e)
            {
                context.TaskCompletionSource.TrySetException(new IOException(SR.ReadAsMimeMultipartErrorReading, e));
            }
        ...
        private static void MultipartWriteSegmentAsync(MultipartAsyncContext context)
        {
            ...
            try
            {
                IAsyncResult result = output.BeginWrite(segment.Array, segment.Offset, segment.Count, onMultipartWriteSegmentAsyncComplete, context);
                if (result.CompletedSynchronously)
                {
            ...
            catch (Exception e)
            {
                ...
                context.TaskCompletionSource.TrySetException(new IOException(SR.ReadAsMimeMultipartErrorWriting, e));
            }
        }
        ...
        private class MultipartAsyncContext
        {
            ...
            public MultipartAsyncContext(HttpContent content, TaskCompletionSource<IEnumerable<HttpContent>> taskCompletionSource, MimeMultipartBodyPartParser mimeParser, byte[] data)
            {
                ...
                this.Result = new List<HttpContent>();
            ...
            /// <summary>
            /// Gets the <see cref="HttpContent"/> that we read from.
            /// </summary>
            /// <value>
