...
namespace System.Json
{
    ...
    public sealed class JsonPrimitive : JsonValue
    {
            ...
            else
            {
                ...
                switch (failure)
                {
                    case ReadAsFailureKind.InvalidFormat:
                        throw Fx.Exception.AsError(new FormatException(SR.CannotReadPrimitiveAsType(valueStr, typeOfTName)));
                    case ReadAsFailureKind.InvalidDateFormat:
                        throw Fx.Exception.AsError(new FormatException(SR.InvalidDateFormat(valueStr, typeOfTName)));
                    case ReadAsFailureKind.InvalidUriFormat:
                        throw Fx.Exception.AsError(new UriFormatException(SR.InvalidUriFormat(jsonPrimitiveType.Name, valueStr, typeOfTName, uriType.Name)));
                    case ReadAsFailureKind.Overflow:
                        throw Fx.Exception.AsError(new OverflowException(SR.OverflowReadAs(valueStr, typeOfTName)));
                    case ReadAsFailureKind.InvalidCast:
                    default:
                        throw Fx.Exception.AsError(new InvalidCastException(SR.CannotReadPrimitiveAsType(valueStr, typeOfTName)));
                }
            }
        }
        ...
        private static char ParseChar(string value, NumberStyles style)
        {
            ...
            {
                throw Fx.Exception.AsError(new InvalidCastException(exception.Message, exception));
            }
