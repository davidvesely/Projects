    ...
    public class IOThreadScheduler
    {
        ...
        bool ScheduleCallbackHelper(Action<object> callback, object state)
        {
            ...
            if (Bits.Count(slot) == -1)
            {
                ...
                throw Fx.AssertAndFailFast("Head/Tail overflow!");
            }
        ...
        bool ScheduleCallbackLowPriHelper(Action<object> callback, object state)
        {
            ...
            if (Bits.CountNoIdle(slot) == 0)
            {
                ...
                // because headTail no longer knows how many work items we have - it looks like zero.  This can
                // only happen if 32k threads come through here while one is swapped out.
                throw Fx.AssertAndFailFast("Low-priority Head/Tail overflow!");
            }
        ...
        struct Slot
        {
            ...
            public bool TryEnqueueWorkItem(Action<object> callback, object state, out bool wrapped)
            {
                ...
                Fx.Assert(this.callbackValue == null, "Slot already has a work item.");
            ...
            public void DebugVerifyEmpty()
            {
                ...
                Fx.Assert(this.callbackValue == null, "Finalized with leaked callback.");
                Fx.Assert(this.stateValue == null, "Finalized with leaked state.");
            }
#endif
        }
        ...
        unsafe class ScheduledOverlapped
        {
            ...
            void IOCallback(uint errorCode, uint numBytes, NativeOverlapped* overlapped)
            {
            ...
            public void Cleanup()
            {
                ...
                {
                    throw Fx.AssertAndFailFast("Cleanup called on an overlapped that is in-flight.");
                }
