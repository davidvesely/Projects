...
namespace System.Net.Http
{
    ...
    /// </summary>
    public static class HttpContentMessageExtensionMethods
    {
        ...
        public static HttpRequestMessage ReadAsHttpRequestMessage(this HttpContent content)
        {
            return ReadAsHttpRequestMessage(content, Uri.UriSchemeHttp, DefaultBufferSize);
        }
        ...
        public static HttpRequestMessage ReadAsHttpRequestMessage(this HttpContent content, string uriScheme)
        {
            return ReadAsHttpRequestMessage(content, uriScheme, DefaultBufferSize);
        }
        ...
        public static HttpRequestMessage ReadAsHttpRequestMessage(this HttpContent content, string uriScheme, int bufferSize)
        {
            if (content == null)
            {
                throw new ArgumentNullException("content");
            ...
            {
                throw new ArgumentException(SR.HttpMessageParserInvalidUriScheme(uriScheme, typeof(Uri).Name), "uriScheme");
            }
            ...
            while (true)
            {
                try
                {
                    bytesRead = content.ReadAsStreamAsync().Result.Read(buffer, 0, buffer.Length);
                }
                catch (Exception e)
                {
                    throw new IOException(SR.HttpMessageErrorReading, e);
                }
                ...
                {
                    // Create and return parsed HttpRequestMessage
                    return CreateHttpRequestMessage(uriScheme, httpRequest, content.ReadAsStreamAsync().Result, bytesRead - headerConsumed);
                }
                else if (parseStatus != ParserState.NeedMoreData)
                {
                    throw new IOException(SR.HttpMessageParserError(headerConsumed, buffer));
                }
            }
        }
        ...
        public static HttpResponseMessage ReadAsHttpResponseMessage(this HttpContent content)
        {
            return ReadAsHttpResponseMessage(content, DefaultBufferSize);
        }
        ...
        /// <returns>The parsed <see cref="HttpResponseMessage"/> instance.</returns>
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "Exception translates to parser state.")]
        public static HttpResponseMessage ReadAsHttpResponseMessage(this HttpContent content, int bufferSize)
        {
            if (content == null)
            {
                throw new ArgumentNullException("content");
            ...
            while (true)
            {
                ...
                if (parseStatus == ParserState.Done)
                {
                    ...
                    return CreateHttpResponseMessage(httpResponse, content.ReadAsStreamAsync().Result, bytesRead - headerConsumed);
                }
                else if (parseStatus != ParserState.NeedMoreData)
                {
                    throw new IOException(SR.HttpMessageParserError(headerConsumed, buffer));
        ...
        private static Uri CreateRequestUri(string uriScheme, HttpUnsortedRequest httpRequest)
        {
            ...
            if (httpRequest.HttpHeaders.TryGetValues(FormattingUtilities.HttpHostHeader, out hostValues))
            {
                ...
                if (hostCount != 1)
                {
                    throw new IOException(SR.HttpMessageParserInvalidHostCount(FormattingUtilities.HttpHostHeader, hostCount));
                }
            }
            else
            {
                throw new IOException(SR.HttpMessageParserInvalidHostCount(FormattingUtilities.HttpHostHeader, 0));
            }
        ...
        private static HttpContent CreateHeaderFields(HttpHeaders source, HttpHeaders destination, Stream contentStream, int rewind)
        {
            ...
            foreach (KeyValuePair<string, IEnumerable<string>> header in source)
            {
                if (FormattingUtilities.HttpContentHeaders.Contains(header.Key))
                {
                    ...
                    contentHeaders.AddWithoutValidation(header.Key, header.Value);
                }
                else
                {
                    destination.AddWithoutValidation(header.Key, header.Value);
                }
            }
            ...
            if (contentHeaders != null)
            {
                ...
                {
                    throw new IOException(SR.HttpMessageContentStreamMustBeSeekable("ContentReadStream", FormattingUtilities.HttpResponseMessageType.Name));
                }
